...
//          cpus params.cpu_candidates
        ...
        then
            ...
            mmseqs createindex  targetDB tmp --threads !{task.cpus}
            rm -rf tmp
        else
...
//      cpus params.cpu_candidates
        ...
        if [ "!{params.modelling_quality}"  ==  "fast" ]
        then
            hfinder.py -q !{fam[3]} -d !{fasta} -db targetDB  -tmp tmp -s mmseqs -e !{params.hfinder_evalue} -lmin !{fam[1]} -lmax !{fam[2]} -b extract fastcheck -r !{fam[0]}_candidates/ -n 1 -t !{task.cpus}
        else
            hfinder.py -q !{fam[3]} -qm !{fam[4]} -d ${fasta} -s blastp hmmsearch ssearch -e !{params.hfinder_evalue} -lmin !{fam[1]} -lmax !{fam[2]} -b extract cumulative check -r !{fam[0]}_candidates/ -n 1 -t !{task.cpus}
        fi
...
//  label 'modelling'
    ...
    shell:
    """
    #runpsipred !{fasta} !{task.cpus}
    mkdir -p ref/!{fasta.baseName}/ best_model_ref/ tneg/!{fasta.baseName}/ best_model_tneg/
    # ref -d \$(pwd)/!{fasta.baseName}.horiz
    modeller_script_singularity.py -l model check -s proq_standalone mypmfs -f !{fasta} -r ref/!{fasta.baseName}/ -pd !{params.database}/!{fam}/!{fam}_ref_pdb.faa -pr !{params.cleaned_pdb} -k !{params.proq} !{params.mypmfs} -j !{params.psipred}/ -q !{params.modelling_quality} -n !{params.model}  -nb !{params.template} -t !{task.cpus}  -pi blastp -smypmfs !{params.trained_data}
    # tneg
    modeller_script_singularity.py -l model check -s proq_standalone mypmfs -f !{fasta} -r tneg/!{fasta.baseName}/ -pd !{params.database}/!{fam}/!{fam}_tneg_pdb.faa -pr !{params.cleaned_pdb} -k !{params.proq} !{params.mypmfs} -j !{params.psipred}/ -q !{params.modelling_quality} -n !{params.model}  -nb !{params.template} -t !{task.cpus}  -pi blastp -smypmfs !{params.trained_data}
...
//  cpus params.cpu
    ...
    PDBRMSD.py -l !{fam} -q !{best_pdb_ref} -t ${params.database}/!{fam}/ref_pdb/ -s TMalign mammoth  -p !{params.tmalign_dir} !{params.mammoth_dir}  -r !{fam}_candidates_ref_vs_ref/ -b 1 -n !{task.cpus}
    PDBRMSD.py -l !{fam} -q !{best_pdb_tneg} -t ${params.database}/!{fam}/tneg_pdb/ -s TMalign mammoth  -p !{params.tmalign_dir} !{params.mammoth_dir}  -r !{fam}_candidates_tneg_vs_tneg/ -b 1 -n !{task.cpus}
    """
}
